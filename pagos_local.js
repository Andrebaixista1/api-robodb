...
import { createClient } from '@supabase/supabase-js'; // Alterado de require para import
...
async function upsertDataPg(df) {
    ...
    try {
        ...
        logging.info("Dados inseridos/atualizados na tabela pg_novo com sucesso!");
    } catch (e) {
...
async function fetchDatapgApi(startDate, endDate) {
    ...
    for (const entry of urls) {
        ...
        let tentativas = 3; // Número de tentativas antes de desistir
        while (tentativas > 0) {
            try {
                ...
                logging.error(`Erro ao acessar a API da empresa ${entry.empresa} (tentativa ${4 - tentativas}): ${e.message}`);
                tentativas--;
                ...
                if (tentativas > 0) {
                    ...
                    logging.error(`Falha ao acessar API da empresa ${entry.empresa} após múltiplas tentativas.`);
                }
            }
        }
    }
